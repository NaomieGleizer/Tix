document.addEventListener("DOMContentLoaded", function (event) {
    ...
    if (sessionStorage.getItem("restaurant_connection_menu")) {
        data = JSON.parse(sessionStorage.getItem("restaurant_connection_menu"));
        show_menu(data);
    }
    // else, sent a request to server to get menu
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                sessionStorage.setItem("restaurant_connection_menu", JSON.stringify(data));
                show_menu(data);
            }
        }
...
function load_menu(data) {
    ...
    var index_row = 0;
    var id = 0;
    var scrollable_menu_div = document.createElement("div");
    scrollable_menu_div.className = "scrollmenu";
    for (i in data.categories) {
        var category_div = document.createElement("div");
        var category = data.categories[i];
        category_div.className = "category_name";
        category_div.id = category.name;
        category_div.innerHTML = category.name;
        a_href_category = document.createElement("a");
        a_href_category.href = "#" + category.name;
...
function show_menu(data) {
    ...
    for (i in data.categories) {
        ...
        //category_tr.setAttribute("id", "category" + id);
        //category_tr.setAttribute("id", String(category.name));
        ...
        //a.appendChild(category_tr);
        //table_menu.appendChild(a);

        table_menu.appendChild(category_tr);
        //catrgory_th.setAttribute("id", "category" + id);

        //var a_tr = document.createElement("tr");
        //var a_div = document.createElement("div");
        //var a = document.createElement("a");
        //a.setAttribute("name", "category" + id);
        //a_div.appendChild(a);
       // a_tr.appendChild(a_div);
        //table_menu.appendChild(a_tr);
        var a_href_category = document.createElement("a");
        //a_href_category.setAttribute("href", "#category" + id);
        a_href_category.setAttribute("href", "#" + String(category.name));
        if (active) {
            a_href_category.setAttribute("class", "active");
            active = false;
        }
        ...
        for (j in category.menu_items) {
            ...
            //a.appendChild(meal_tr);
            //a.appendChild(adding_meal_tr);
            //table_menu.appendChild(a);
            id++;
        }
    }
